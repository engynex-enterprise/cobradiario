...
export class OrganizationResolver {
  ...
  /** Permisos efectivos del usuario actual (para gating de UI en web y móvil). */
  @Query(() => [String], { name: 'myPermissions' })
  async myPermissions(@CurrentUser() user: AuthContext): Promise<string[]> {
    const set = await this.permissions.forUser(user.tenantId, user.role);
    return [...set];
  }
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN)
  @RequirePermissions('org_settings')
  ...
  @Roles(UserRole.OWNER, UserRole.ADMIN)
  @RequirePermissions('delete_members')
  @Mutation(() => [OrgMemberModel])
